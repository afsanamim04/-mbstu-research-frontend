...
function App() {
  ...
  // ✅ SIMPLE FIX: Remove API call, use only localStorage
  useEffect(() => {
    const token = localStorage.getItem("authToken");
    const profileName = localStorage.getItem("profileName");
    
    if (token && profileName) {
      // Directly use localStorage data without API call
      setUser({
        name: profileName,
    ...
    <BrowserRouter>
      ...
      <div style={{ maxWidth: 650, margin: '32px auto', padding: 24 }}>
        <Routes>
          ...
          <Route path="/login" element={<Login setUser={setUser} />} /> {/* ✅ FIXED: setUser prop pass */}
